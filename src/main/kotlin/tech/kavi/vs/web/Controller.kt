...
abstract class Controller(val handlers: Router.() -> Unit = {}) {
    ...
    private fun loadController(router: Router) {
        try {
            ...
            if (handlerPackages != null) {
                ...
                // 获取所有请求handler类
                LauncherVerticle.context.getBeansWithAnnotation(HandlerRequest::class.java).filter { (_, beanHandler) ->
                    ...
                    this.loadHandler(router, beanHandler, this.substrPath(beanHandler, packagePath, handlerPackages))
                }
            }
        } catch (e: Exception) {
     ...
     * */
    private fun substrPath(beanHandler: Any, packagePath: String, handlerScan:HandlerScan): String {
        if (handlerScan.isPrefixPath) return SLASH
